...
2. **Step II: Sector Identification**: Consult [The Never List Encyclopedia](auditor-never_list_encyclopedia.md) to identify the Architectural Sector.
...
| :--- | :--- | :--- |
| [auditor_audit_signals.py](../scripts/auditor_audit_signals.py) | String-Signal Decay | Detects legacy `.connect("string", ...)` calls that bypass compile-time validation. |
| [auditor_audit_memory_fragmentation.gd](../scripts/auditor_audit_memory_fragmentation.gd) | ObjectDB / orphans | Snapshot/diff helpers for `OBJECT_COUNT` and orphan node regressions. |
| [auditor_purge_report_generator.gd](../scripts/auditor_purge_report_generator.gd) | Purge summary | Aggregates orphan, unused-resource, and dependency slop into a remediation report. |
| [auditor_audit_type_hints.py](../scripts/auditor_audit_type_hints.py) | Type safety / string connect | Flags untyped `Array`/`Dictionary` and legacy `.connect("string", ...)` decay. |
...
| :--- | :--- | :--- | :--- |
| Signal decay, lambda leaks, string `.connect` | Sector II (Mind) + V (Voice) | [signal-architecture](signal-architecture.md), [gdscript-mastery](gdscript-mastery.md) | **MANDATORY** [auditor_audit_signals.py](../scripts/auditor_audit_signals.py) |
| ObjectDB orphans, memory spikes, purge brief | Sector VI (Shield) | [debugging-profiling](debugging-profiling.md) | **MANDATORY** [auditor_audit_memory_fragmentation.gd](../scripts/auditor_audit_memory_fragmentation.gd) + [auditor_purge_report_generator.gd](../scripts/auditor_purge_report_generator.gd) |
| Untyped Array/Dictionary, Variant hot loops | Sector II (Mind) | [gdscript-mastery](gdscript-mastery.md) | **MANDATORY** [auditor_audit_type_hints.py](../scripts/auditor_audit_type_hints.py) |
| Export case-sensitivity, RCE (`Expression.execute`) | Sector VI (Shield) | [export-builds](export-builds.md) | Category decree only (no scanner) |
| Sector never-list (genre, UI, networking, etc.) | Match sector in [encyclopedia index](auditor-never_list_encyclopedia.md) | One `references/categories/<topic>.md` | Category + optional scanners above |
...
- **The Aurelius Rule**: Symbols over Strings. Always. **MANDATORY** [auditor_audit_signals.py](../scripts/auditor_audit_signals.py) when scanning for decay.
...
**MANDATORY** [auditor_audit_memory_fragmentation.gd](../scripts/auditor_audit_memory_fragmentation.gd). Diff ObjectDB / `Performance.OBJECT_COUNT` / orphan monitors across scene transitions.

### 5. Purge-Report-Generator
**MANDATORY** [auditor_purge_report_generator.gd](../scripts/auditor_purge_report_generator.gd) when producing a prioritized remediation brief.
...
#### Prerequisites
- [godot-project-foundations](project-foundations.md) — Folder layout, naming, and project settings Aurelius surveys before any sector never-list loads.
- [godot-gdscript-mastery](gdscript-mastery.md) — Static typing, warnings, and VM idioms that turn Variant/container slop into enforceable rules.
- [godot-signal-architecture](signal-architecture.md) — Typed Signal.connect, disconnect lifecycle, and bus topology the signal-decay arsenal assumes.
...
- [godot-debugging-profiling](debugging-profiling.md) — ObjectDB / script profiler workflows that turn Aurelius findings into measured proof.
- [godot-performance-optimization](performance-optimization.md) — WorkerThreadPool, RID batching, and draw-call remediation after main-thread or material audits.
- [godot-composition](composition.md) — Component decomposition targets when cyclomatic complexity / god-function checks fire.
- [godot-autoload-architecture](autoload-architecture.md) — Typed singleton ownership and orphan risks when state lives in AutoLoads.
- [godot-resource-data-patterns](resource-data-patterns.md) — local_to_scene, RefCounted ownership, and ResourceLoader dependency hygiene.
- [godot-scene-management](scene-management.md) — PackedScene.get_state introspection and NodePath integrity without instantiating @tool scenes.
- [godot-testing-patterns](testing-patterns-expert-testing-patterns.md) — Verification suites that prove a never-list fix stayed fixed after remediation.

#### Downstream / consumers
- [godot-export-builds](export-builds.md) — Case-sensitive asset names and debug-strip discipline become ship blockers on Linux/Android exports.
- [godot-analyst](analyst.md) — Rubric/scoring peer that grades project quality after Aurelius returns structural decrees.
- [godot-monte-carlo-balancer](monte-carlo-balancer.md) — When audit slop is balance-coupled (spawn density, economy ticks), simulate impact before accepting a fix.
