...
Sidechain Music bus down while Voice plays — [audio_bus_ducker_logic.gd](../scripts/audio_systems_audio_bus_ducker_logic.gd). Route VO to Voice bus, not SFX.

## Vertical stems (`AudioStreamSynchronized`)

Fade with `set_sync_stream_volume(index, db)` over 1–2s Tweens driven by a 0–1 intensity signal — [audio_interactive_music_manager.gd](../scripts/audio_systems_audio_interactive_music_manager.gd). Deep rules: [interactive-music-deep-dive.md](audio-systems-interactive-music-deep-dive.md).

## Horizontal clips (`AudioStreamInteractive`)

Combat ↔ explore transitions on bar boundaries — [interactive_music_graph.gd](../scripts/audio_systems_interactive_music_graph.gd).
...
Reverb/low-pass on SFX bus for underwater or room zones — [audio_environmental_reverb_zone.gd](../scripts/audio_systems_audio_environmental_reverb_zone.gd). Occlusion via ray + `attenuation_filter_cutoff_hz` — [audio_occlusion_raycast.gd](../scripts/audio_systems_audio_occlusion_raycast.gd).
