...
- Bus/effects model already 3.x-shaped; still re-validate players after convert.
- `Listener` → `AudioListener3D`; positional players renamed with 2D/3D suffixes where applicable.
- `VideoPlayer` → `VideoStreamPlayer`.
- Re-check `AudioServer.device` → `output_device`.
