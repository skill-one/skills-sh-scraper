...
5. Verify before ship: `go test -count=1 ./...` (covers the codec and the generated Tier-1 path without relying on a warm test cache), `go build -tags ble_live ./...` compiles, and the live commands actuate on hardware when available.
