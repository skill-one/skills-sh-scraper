...
from huaweicloudsdkbms.v1.model import ShowWindowsBaremetalServerPwdRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询Windows裸金属服务器初始密码")
...
try:
    ...
    # 构建请求
    request = ShowWindowsBaremetalServerPwdRequest()
    request.server_id = args.server_id

    response = client.show_windows_baremetal_server_pwd(request)

    # 渲染
    output = f"password: {getattr(response, 'password', '')}\n"

    print(output)
except Exception as e:
    print(f"bms.show_windows_baremetal_server_pwd 查询失败: {e}")
    exit(1)
