...
from huaweicloudsdkbms.v1.model import ShowBaremetalServerTagsRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询裸金属服务器标签")
...
try:
    ...
    # 构建请求
    request = ShowBaremetalServerTagsRequest()
    request.server_id = args.server_id

    response = client.show_baremetal_server_tags(request)
    tags = response.tags

    if not tags:
        print(f"没有找到标签 (区域: {Region}, 服务器ID: {args.server_id})")
        exit(0)
    ...
    print(f"bms.show_baremetal_server_tags 查询失败: {e}")
    exit(1)
