...
from huaweicloudsdkas.v1 import AsClient, ListScalingTagInfosByResourceIdRequest
...
parser = argparse.ArgumentParser(description='按资源ID查询标签')
...
def main():
    ...
    try:
        ...
        request = ListScalingTagInfosByResourceIdRequest()
        request.resource_type = args.resource_type
        request.resource_id = args.resource_id

        response = client.list_scaling_tag_infos_by_resource_id(request)
        ...
        if not tags and not sys_tags:
            print(f"没有找到标签 (区域: {Region})")

    except Exception as e:
        print(f"as.list_scaling_tag_infos_by_resource_id 查询失败: {e}")
        exit(1)
