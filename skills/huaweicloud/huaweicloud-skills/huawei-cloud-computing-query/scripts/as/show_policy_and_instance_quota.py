...
from huaweicloudsdkas.v1 import AsClient, ShowPolicyAndInstanceQuotaRequest
...
parser = argparse.ArgumentParser(description='查询伸缩组策略和实例配额')
...
def main():
    ...
    try:
        ...
        request = ShowPolicyAndInstanceQuotaRequest()
        request.scaling_group_id = args.scaling_group_id

        response = client.show_policy_and_instance_quota(request)
        ...
        print(f"as.show_policy_and_instance_quota 查询失败: {e}")
        exit(1)
