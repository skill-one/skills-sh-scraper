...
parser = argparse.ArgumentParser(description="查询 IAM 用户附加策略列表 (v5)")
...
def render(policies, total_count=None, has_more=False, next_marker=None):
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条 IAM 用户附加策略，当前返回 {showing_count} 条"
...
try:
    ...
    if not policies:
        print(f"没有找到 IAM 用户附加策略 (区域: {Region})")
        exit(0)
    ...
    print(f"iam.list_attached_user_policies_v5 查询失败: {e}")
    exit(1)
