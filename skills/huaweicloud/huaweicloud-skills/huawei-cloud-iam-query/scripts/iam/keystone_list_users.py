...
parser = argparse.ArgumentParser(description="查询用户列表 (v3)")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--domain_id", type=str, help="域 ID")
parser.add_argument("--name", type=str, help="用户名")
parser.add_argument("--enabled", type=str, help="是否启用 true/false")
parser.add_argument("--password_expires_at", type=str, help="密码过期时间过滤，格式: YYYY-MM-DD 或运算符:YYYY-MM-DD，运算符支持 lt/lte/gt/gte/eq/neq")
...
try:
    ...
    if args.enabled:
        request.enabled = args.enabled.lower() == 'true'
    if args.password_expires_at:
        request.password_expires_at = args.password_expires_at
    ...
    if has_more:
        ...
        if next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --domain_id / --name / --enabled 等参数缩小查询范围"
    else:
        output += f"\n共 {len(display_items)} 条"

    print(output)
except Exception as e:
    print(f"iam.keystone_list_users 查询失败: {e}")
    exit(1)
