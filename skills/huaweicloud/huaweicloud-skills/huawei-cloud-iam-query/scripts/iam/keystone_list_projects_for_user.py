...
from huaweicloudsdkiam.v3.model import KeystoneListProjectsForUserRequest
...
parser = argparse.ArgumentParser(description="查询用户可访问项目 (v3)")
...
try:
    ...
    response = client.keystone_list_projects_for_user(request)
    ...
    output = f"id	name	domain_id	enabled	description	parent_id\n"
    ...
    print(f"iam.keystone_list_projects_for_user 查询失败: {e}")
    exit(1)
