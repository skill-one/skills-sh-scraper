...
parser = argparse.ArgumentParser(description="查询委托所有项目权限 (v3)")
...
try:
    ...
    request = ListAllProjectsPermissionsForAgencyRequest()
    request.agency_id = args.agency_id
    if args.domain_id:
        request.domain_id = args.domain_id
    response = client.list_all_projects_permissions_for_agency(request)
    ...
    print(f"iam.list_all_projects_permissions_for_agency 查询失败: {e}")
    exit(1)
