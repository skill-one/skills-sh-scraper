...
from huaweicloudsdkiam.v3.model import KeystoneListVersionsRequest
...
parser = argparse.ArgumentParser(description="查询 Keystone 版本列表 (v3)")
...
try:
    ...
    request = KeystoneListVersionsRequest()

    response = client.keystone_list_versions(request)
    versions_obj = response.versions
    if not versions_obj:
        print(f"没有找到数据 (区域: {Region})")
        exit(0)
    items = versions_obj.values
    if not items:
        print(f"没有找到数据 (区域: {Region})")
        exit(0)
    ...
    print(f"iam.keystone_list_versions 查询失败: {e}")
    exit(1)
