...
from huaweicloudsdkiam.v3.model import ListAgenciesRequest
...
parser = argparse.ArgumentParser(description="查询委托列表 (v3)")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--domain_id", type=str, help="委托方域 ID")
parser.add_argument("--trust_domain_id", type=str, help="被委托方域 ID")
parser.add_argument("--name", type=str, help="委托名称")
...
try:
    ...
    if args.trust_domain_id:
        request.trust_domain_id = args.trust_domain_id
    if args.name:
    ...
    # 只做一次查询
    response = client.list_agencies(request)
    ...
    output = f"id	name	domain_id	trust_domain_id	trust_domain_name	description	duration	expire_time	agency_urn	created_at\n"
    ...
    print(f"iam.list_agencies 查询失败: {e}")
    exit(1)
