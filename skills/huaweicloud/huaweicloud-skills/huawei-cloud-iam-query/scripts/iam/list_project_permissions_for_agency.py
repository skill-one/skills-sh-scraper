...
from huaweicloudsdkiam.v3.model import ListProjectPermissionsForAgencyRequest
...
parser = argparse.ArgumentParser(description="查询 IAM 委托项目权限列表 (v3)")
...
def render(roles, total_count=None, has_more=False, next_marker=None):
    """
    渲染权限列表
    :param roles: 权限列表（最多 PAGE_SIZE 条）
    :param total_count: 总数，None 表示未知
    :param has_more: 是否还有更多数据
    :param next_marker: 下一页的 marker
    """
    if not roles:
        print("没有找到 IAM 委托项目权限")
        return

    output = f"id\tname\tdisplay_name\ttype\tcatalog\tdescription\tflag\tdomain_id\tupdated_time\tcreated_time\n"
    for item in roles:
        id_ = getattr(item, 'id', '')
        ...
        output += f"{id_}\t{name}\t{display_name}\t{type_}\t{catalog}\t{description}\t{flag}\t{domain_id}\t{updated_time}\t{created_time}\n"

    # 汇总信息
    showing_count = len(roles)

    if total_count is not None:
        output += f"\n共 {total_count} 条 IAM 委托项目权限，当前返回 {showing_count} 条"
...
try:
    ...
    response = client.list_project_permissions_for_agency(request)
    items = response.roles or []

    if not items:
        print(f"没有找到 IAM 委托项目权限 (区域: {Region}, 项目 ID: {args.project_id}, 委托 ID: {args.agency_id})")
        exit(0)
    ...
    print(f"iam.list_project_permissions_for_agency 查询失败: {e}")
    exit(1)
