...
parser = argparse.ArgumentParser(description="查询 IAM 委托列表 (v5)")
...
try:
    ...
    # 只做一次查询
    response = client.list_agencies_v5(request)
    items = response.agencies
    if not items:
        print(f"没有找到 IAM 委托 (区域: {Region})")
        exit(0)
    ...
    output = f"agency_id\tagency_name\ttrust_domain_id\ttrust_domain_name\tdescription\tcreated_at\n"
    ...
    if has_more:
        ...
        if next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --path_prefix 等参数缩小查询范围"
        else:
            output += f"\n可使用 --path_prefix 等参数缩小查询范围"
    else:
        output += f"\n共 {showing_count} 条"

    print(output)
except Exception as e:
    print(f"iam.list_agencies_v5 查询失败: {e}")
    exit(1)
