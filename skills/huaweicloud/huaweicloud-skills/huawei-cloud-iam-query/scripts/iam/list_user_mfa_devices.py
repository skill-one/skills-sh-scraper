...
parser = argparse.ArgumentParser(description="查询 IAM 用户 MFA 设备列表 (v3)")
...
def render(devices, total_count=None, has_more=False, next_marker=None):
    ...
    """
    if not devices:
        print("没有找到 IAM 用户 MFA 设备")
        return

    output = f"user_id\tserial_number\n"
    for item in devices:
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条 IAM 用户 MFA 设备，当前返回 {showing_count} 条"
        ...
        output += f"\n共 {showing_count} 条 IAM 用户 MFA 设备"
...
try:
    ...
    response = client.list_user_mfa_devices(request)
    items = response.virtual_mfa_devices or []

    if not items:
        print(f"没有找到 IAM 用户 MFA 设备 (区域: {Region})")
        exit(0)
    ...
    if args.marker:
        for i, item in enumerate(items):
            if str(getattr(item, 'serial_number', '')) == args.marker:
                start_idx = i + 1
                break
    ...
    next_marker = None
    if has_more:
        next_marker = str(getattr(remaining_items[PAGE_SIZE - 1], 'serial_number', ''))
    ...
    print(f"iam.list_user_mfa_devices 查询失败: {e}")
    exit(1)
