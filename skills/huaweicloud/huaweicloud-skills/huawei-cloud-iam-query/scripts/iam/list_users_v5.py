...
parser = argparse.ArgumentParser(description="查询 IAM 用户列表 (v5)")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--group_id", type=str, help="用户组 ID，可通过 list_groups_v5.py 获取")
...
def render(users, total_count=None, has_more=False, next_marker=None):
    ...
    output = f"user_id\tuser_name\tenabled\tdescription\tcreated_at\n"
    for item in users:
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条 IAM 用户，当前返回 {showing_count} 条"
        if has_more and next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --group_id 等参数缩小查询范围"
    elif has_more:
        output += f"\n当前返回 {showing_count} 条，还有更多数据"
        if next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --group_id 等参数缩小查询范围"
    else:
        output += f"\n共 {showing_count} 条 IAM 用户"
...
try:
    ...
    # 只展示前 PAGE_SIZE 条
    display_users = users[:PAGE_SIZE]

    # 渲染结果
    render(display_users, has_more=has_more, next_marker=next_marker)
except Exception as e:
    print(f"iam.list_users_v5 查询失败: {e}")
    exit(1)
