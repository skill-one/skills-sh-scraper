...
from huaweicloudsdkiam.v3.model import KeystoneListServicesRequest
...
parser = argparse.ArgumentParser(description="查询服务列表 (v3)")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--type", type=str, help="服务类型，用于过滤，如 compute、volume 等")
...
try:
    ...
    request = KeystoneListServicesRequest()
    if args.type:
        request.type = args.type
    response = client.keystone_list_services(request)
    ...
    output = f"id	name	type	enabled	description\n"
    ...
    if has_more:
        ...
        if next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --type 等参数缩小查询范围"
    else:
        output += f"\n共 {len(display_items)} 条"

    print(output)
except Exception as e:
    print(f"iam.keystone_list_services 查询失败: {e}")
    exit(1)
