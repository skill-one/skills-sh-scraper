...
from huaweicloudsdkiam.v3.model import KeystoneListProtocolsRequest
...
parser = argparse.ArgumentParser(description="查询协议列表 (v3)")
...
try:
    ...
    request = KeystoneListProtocolsRequest()
    request.idp_id = args.idp_id
    response = client.keystone_list_protocols(request)
    items = response.protocols
    if not items:
    ...
    print(f"iam.keystone_list_protocols 查询失败: {e}")
    exit(1)
