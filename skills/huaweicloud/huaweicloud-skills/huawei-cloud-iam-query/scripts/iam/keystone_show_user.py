...
parser = argparse.ArgumentParser(description="查询用户详情 (v3 Keystone)")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--user_id", type=str, required=True, help="用户 ID，可通过 keystone_list_users.py 获取")
...
try:
    ...
    response = client.keystone_show_user(request)
    item = response.user
    if not item:
        print("没有找到数据")
        exit(0)

    output = f"id	name	domain_id	enabled	description	pwd_status	last_project_id	password_expires_at	access_mode	links\n"
    id = getattr(item, 'id', '')
    ...
    last_project_id = getattr(item, 'last_project_id', '')
    ...
    print(f"iam.keystone_show_user 查询失败: {e}")
    exit(1)
