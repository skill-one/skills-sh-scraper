...
parser = argparse.ArgumentParser(description="查询 IAM 委托全局权限列表 (v3)")
...
def render(roles, total_count=None, has_more=False, next_marker=None):
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条 IAM 委托全局权限，当前返回 {showing_count} 条"
...
try:
    ...
    request = ListDomainPermissionsForAgencyRequest()
    request.domain_id = args.domain_id
    request.agency_id = args.agency_id
    response = client.list_domain_permissions_for_agency(request)
    items = response.roles or []

    if not items:
        print(f"没有找到 IAM 委托全局权限 (区域: {Region}, 域 ID: {args.domain_id}, 委托 ID: {args.agency_id})")
        exit(0)
    ...
    print(f"iam.list_domain_permissions_for_agency 查询失败: {e}")
    exit(1)
