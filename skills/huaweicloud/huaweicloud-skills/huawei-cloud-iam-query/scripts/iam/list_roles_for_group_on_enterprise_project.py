...
from huaweicloudsdkiam.v3.model import ListRolesForGroupOnEnterpriseProjectRequest
...
parser = argparse.ArgumentParser(description="查询用户组在企业项目上的角色 (v3)")
...
def render(roles, total_count=None, has_more=False, next_marker=None):
    ...
    if total_count is not None:
        output += f"\n共 {total_count} 条用户组在企业项目上的角色，当前返回 {showing_count} 条"
...
try:
    ...
    request = ListRolesForGroupOnEnterpriseProjectRequest()
    request.enterprise_project_id = args.enterprise_project_id
    request.group_id = args.group_id
    response = client.list_roles_for_group_on_enterprise_project(request)
    ...
    print(f"iam.list_roles_for_group_on_enterprise_project 查询失败: {e}")
    exit(1)
