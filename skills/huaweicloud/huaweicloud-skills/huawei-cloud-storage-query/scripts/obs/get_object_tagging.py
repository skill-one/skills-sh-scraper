...
from huaweicloudsdkobs.v1.model import GetObjectTaggingRequest
from huaweicloudsdkobs.v1.region.obs_region import ObsRegion

AK, SK, Region, SecurityToken = load_credentials()

parser = argparse.ArgumentParser(description="获取对象标签")
...
try:
    ...
    response = client.get_object_tagging(request)

    tagging = getattr(response, 'tagging', None)

    if not tagging:
        print("查询结果为空")
        exit(0)

    tag_set = getattr(tagging, 'tag_set', None)
    tags = getattr(tag_set, 'tag', []) or [] if tag_set else []

    if not tags:
        print("查询结果为空")
        exit(0)

    output = f"对象 {args.object_key} 的标签列表:\n"
    output += "键\t值\n"
    ...
    print(f"obs.get_object_tagging 查询失败: {e}")
    exit(1)
