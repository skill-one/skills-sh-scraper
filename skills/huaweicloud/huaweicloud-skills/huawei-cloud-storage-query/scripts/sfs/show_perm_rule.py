...
from huaweicloudsdksfsturbo.v1.model import ShowPermRuleRequest
from huaweicloudsdksfsturbo.v1.region.sfsturbo_region import SFSTurboRegion

AK, SK, Region, SecurityToken = load_credentials()

parser = argparse.ArgumentParser(description="查询文件系统权限规则详情")
...
parser.add_argument("--rule_id", type=str, required=True, help="规则ID")
...
try:
    request = ShowPermRuleRequest()
    request.share_id = args.share_id
    request.rule_id = args.rule_id
    ...
    response = client.show_perm_rule(request)

    rule_id = getattr(response, 'id', '')
    ip_cidr = getattr(response, 'ip_cidr', '')
    rw_type = getattr(response, 'rw_type', '')
    user_type = getattr(response, 'user_type', '')

    output = "文件系统权限规则详情:\n"
    output += f"规则ID: {rule_id}\n"
    output += f"IP CIDR: {ip_cidr}\n"
    output += f"读写类型: {rw_type}\n"
    output += f"用户类型: {user_type}\n"

    print(output.strip())
    exit(0)
except Exception as e:
    print(f"sfs.show_perm_rule 查询失败: {e}")
    exit(1)
