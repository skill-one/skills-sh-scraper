...
try:
    ...
    job_type = getattr(response, 'job_type', '')

    output = "任务详情:\n"
    output += f"任务ID: {job_id}\n"
    output += f"状态: {status}\n"
    output += f"任务类型: {job_type}\n"

    print(output.strip())
    exit(0)
except Exception as e:
    print(f"sfs.show_job_detail 查询失败: {e}")
    exit(1)
