...
> ✅ 真正流式输出，生成一张返回一张
> ✅ 提示词智能优化，自动提升出图质量
> ✅ 自动按日期归档到 Claude 启动目录 + 完整 metadata 记录
...
```bash
# 在你想保存图片的目录下运行，用脚本绝对路径调用（不要 cd 进 skill 目录）
node ~/.agents/skills/byted-ark-seedream-skill/scripts/generate.js --prompt "一只可爱的英短蓝猫趴在窗边晒太阳" --size 2K
```

看到图片成功保存到当前目录的 `Seedream-Images/`，说明安装成功！
...
### Q：图片保存在哪里？
A：默认保存在 **Claude 启动目录**（脚本运行时的当前工作目录）下的 `Seedream-Images` 文件夹，按日期子目录归档，同时生成完整的 metadata JSON 文件记录所有生成参数。如需固定到其它位置，可设置环境变量 `ARK_SEEDREAM_SAVE_PATH`。
