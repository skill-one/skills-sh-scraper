...
| 优先级 | 路径 | 说明 |
|-------|------|------|
| 1 | `ARK_SEEDREAM_SAVE_PATH` / `ARK_SAVE_PATH` | 环境变量显式指定固定路径时 |
| 2 | `<Claude 启动目录>/Seedream-Images/` | 默认，取自 `process.cwd()`（脚本运行时的当前工作目录） |

> ⚠️ 默认目录取自 `process.cwd()`，请用脚本【绝对路径】在启动目录调用，**不要 `cd` 进 skill 目录**，否则图片会存到 skill 目录里。
...
| `ARK_SEEDREAM_SAVE_PATH` | 图片保存路径 | Claude 启动目录（当前工作目录） |
