# 상표 사용 법적 고지 — `coupang-product-search`

이 스킬에서 `쿠팡` 및 `Coupang` 명칭은 쿠팡 상품 검색·가격·배송 조건 조회의 **대상 서비스**를 식별하기 위해 사용한다. k-skill 또는 upstream 호환 도구의 출처를 쿠팡 운영사로 표시하려는 사용이 아니다.
