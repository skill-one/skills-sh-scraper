# 상표 사용 법적 고지 — `olive-young-search`

이 스킬에서 `올리브영` 및 `Olive Young` 명칭은 매장·상품·재고 조회의 **대상 서비스**를 식별하기 위해 사용한다. k-skill 또는 upstream CLI의 출처를 올리브영 운영사로 표시하려는 사용이 아니다.
...
따라서 이 명칭은 필요한 범위의 평문으로만 사용하고, 별도 근거 없이 로고, 공식·제휴·후원·인증·파트너 표현을 사용하지 않는다. 각 상표의 권리는 해당 권리자에게 있다. 이 문서는 상품정보의 저작권·데이터베이스권, 부정경쟁방지법, 약관·계약 또는 접근·재사용의 적법성을 판단하지 않는다.
