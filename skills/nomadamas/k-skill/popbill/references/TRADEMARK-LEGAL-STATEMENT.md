...
이 스킬에서 `팝빌` 및 `Popbill` 명칭은 전자세금계산서·전자명세서·현금영수증·메시징 등 **대상 API 서비스**를 식별하기 위해 사용한다. k-skill의 출처를 팝빌 운영사로 표시하려는 사용이 아니다.
...
따라서 이 명칭은 필요한 범위의 평문으로만 사용하고, 별도 근거 없이 로고, 공식·제휴·후원·인증·파트너 표현을 사용하지 않는다. 각 상표의 권리는 해당 권리자에게 있다. 이 문서는 API 계약, 세무·전자문서 규제, 메시지 발송 권한, 개인정보 또는 개별 거래의 적법성을 판단하지 않는다.
