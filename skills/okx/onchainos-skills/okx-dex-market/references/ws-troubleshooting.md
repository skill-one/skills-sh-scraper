...
Error handling and edge cases for the WS capability. Load the operational flow through the top-level WS core route.
...
- **Missing required parameter on `ws start`**: each channel has its own required params (see the Channel Quick Reference in the WS core reference) — `--token-pair` for per-token channels, `--chain-index` for per-chain channels, `--wallet-addresses` for the address tracker. `--token-pair` format is `chainIndex:tokenContractAddress`.
- **`--wallet-addresses` cap**: max 200 addresses per session — split larger sets across sessions.
- **Custom script cannot connect / auth fails**: verify the endpoint is `wss://wsdex.okx.com/ws/v6/dex`, HMAC-SHA256 login is performed before subscribing, and a `"ping"` heartbeat is sent every 25s (server closes idle unauthenticated connections). Load the full protocol for the selected channel group through the top-level conditional-reference table.
