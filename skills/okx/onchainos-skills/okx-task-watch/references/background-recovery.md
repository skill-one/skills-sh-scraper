...
> Loaded from `SKILL.md` §Anti-patterns only when a watch call accidentally ended up in the background. Rare error path — not part of the normal loop.
...
2. Parse the JSON items and dispatch each per `SKILL.md` §Dispatch (paste blockquote for notifications; paste blockquote for decision_requests; same rules, same no-thinking ban).
...
4. Restart watch **in the foreground** per `SKILL.md` §Run watch (preserving sticky `--job-id <X>` per §Session-scoped sticky if applicable).
