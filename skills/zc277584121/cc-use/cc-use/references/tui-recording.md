...
( asciinema rec -y --overwrite \
    -c "timeout 50 tmux -S \"$HOME/.cc-use/tmux/cc-use\" attach -r -t '=SESSION_NAME'" \
    --idle-time-limit 2 \
...
- 必须使用 `tmux -S "$HOME/.cc-use/tmux/cc-use" attach -r`，连接专用 socket 并避免录制端意外向内层 TUI 输入内容。
...
```bash
tmux -S "$HOME/.cc-use/tmux/cc-use" set-option -t '=SESSION_NAME' window-size manual
tmux -S "$HOME/.cc-use/tmux/cc-use" resize-window -t '=SESSION_NAME' -x 120 -y 36
```

录制结束后恢复：

```bash
tmux -S "$HOME/.cc-use/tmux/cc-use" set-option -t '=SESSION_NAME' window-size latest
```
