...
def generate_composition_stub(
    section, video_title, output_dir, short_timing, section_title
):
...
def _run(args, started_at):
    ...
    for sec in qualifying:
        ...
        comp_id = generate_composition_stub(
            sec, args.title, output_dir, short_timing, section_title
        )
    ...
    if not result["generated"] and result["failed"]:
        sys.exit(
            cli_envelope.emit_error(
                args,
                "ffmpeg_failed"
                if all(f["stage"] == "extract_audio" for f in result["failed"])
                else "internal_error",
    ...
    print(
        "     (NOT the long-form videos/{name}/ — shorts have their own audio + timing)"
    )
