...
def analyse_content(text):
    ...
    if cn_chars == 0:
        ...
        reason = (
            f"Mostly Chinese ({cn_chars} chars) with {bare_abbrev_count} bare abbreviations — "
...
def print_advisory(text, current_voice):
    ...
    print(
        f"  English tokens: {metrics['en_tokens']} "
    ...
    if (
        current_voice
        and "Multilingual" in current_voice
        and metrics["recommendation"] == "standard"
    ):
        ...
        current_voice
        and "Multilingual" not in current_voice
        and metrics["recommendation"] == "multilingual"
    ):
        print("  ⚠️  Current voice is standard but content has substantial English.")
        print(
            f"      Consider switching to {suggested} for better English pronunciation."
        )
    return metrics
