...
def validate_sections(text, sections, matches):
    ...
    if pre_content:
        warnings.append(
            f"Content before first section marker will be included but not section-mapped: '{pre_content[:50]}...'"
        )
    ...
    if len(sections) > 1 and word_boundaries:
        ...
        for sec_idx, section in enumerate(sections[1:], 1):
            ...
            for i in range(search_start, len(word_boundaries)):
                ...
                for j in range(i, min(i + 30, len(word_boundaries))):
                    ...
                    if len(window_clean) >= 10 and window_clean.startswith(
                        target_clean[:12]
                    ):
                        ...
                        search_start = i + 1
                        print(
                            f'  ✓ {section["name"]}: {section["start_time"]:.2f}s (matched: "{window[:20]}...")'
                        )
                        found = True
                        break
                if found:
            ...
            if not found:
                ...
                sections[last_anchored]["end_time"] = section["start_time"]
                print(
                    f'  ⚠ {section["name"]}: {section["start_time"]:.2f}s (estimated, not found: "{target_clean[:15]}")'
                )
        ...
        for i in range(len(sections) - 1, -1, -1):
            if sections[i].get("is_silent", False):
                ...
                print(
                    f"  ℹ {sections[i]['name']}: silent section, Remotion adds extra frames"
                )
            else:
