...
const DataBarRow = ({
    ...
    // Hooks at component top level — never inside a .map() (Rules of Hooks).
    const pct = (item.value / max) * 100;
    const a = useEntrance(props.enableAnimations, delay + index * 5);
    const fillPct = useBarFill(pct, delay + index * 5 + 10);
    return (
        <div
            style={{
                display: "flex",
    ...
    return (
        <div
            style={{
                ...
                padding: "32px 36px",
                boxShadow:
                    "0 2px 8px rgba(0,0,0,0.03), 0 4px 16px rgba(0,0,0,0.04)",
            }}
        >
            {items.map((item, i) => (
                <DataBarRow
