...
description: |
  ...
  AI 詞彙、否定式排比、過多的連接性短語。若使用者要求清理不可見 Unicode、
  文字型 AI 浮水印或檔案 provenance，改用 $text-watermark-cleaner-zh-tw，
  不要在本 skill 中把語氣改寫冒充成浮水印已移除。
allowed-tools:
  - Read
  - Write
  - Edit
...
0. **先分類需求** - 只要求去 AI 味就留在本 skill；要求文字浮水印、不可見字元或 provenance 時，交給 `$text-watermark-cleaner-zh-tw`
1. **保護非 prose 區段** - 不改動程式碼、URL、路徑、API 名稱、引用、數字、必要揭露與逐字引用
2. **辨識 AI 模式** - 掃瞄下面列出的模式
3. **重寫問題片段** - 用自然的替代方案替換 AI 痕跡
4. **保留含義** - 保持核心資訊、事實、觀點與不確定性完整
5. **維持語調** - 匹配預期的語氣（正式、隨意、技術等）
6. **增加作者聲音** - 只能從原文或使用者提供的素材提煉個性，不虛構經歷或背景

## 與文字浮水印技能的分工

`humanizer-zh-tw` 處理語氣、結構與 AI 寫作模式；`text-watermark-cleaner-zh-tw` 處理不可見 Unicode 與文字 provenance。兩者不是同一種證據：自然改寫不等於浮水印已移除，Unicode 清理也不會移除統計式 token watermark。

當使用者同時要求兩者時，遵循：

```text
保護非 prose 區段 → 文字浮水印 skill 的 inspect/Layer A → 本 skill 人性化 → 可選 Layer B → Layer A 再檢查
```
...
6. **不新增事實** - 不自行加入第一人稱經驗、數字、來源、回饋、對話或背景故事
...
**適當使用「我」。** 第一人稱不是不專業，但只能在原文已有第一人稱，或使用者明確提供可使用的經驗時保留或調整。不要為了製造人味而新增「我上週……」等內容。

**保留真實的混亂。** 不要把原作者的停頓、保留態度或不規則節奏全部磨平；也不要自行添加離題、題外話或半成形的想法。
...
1. 仔細閱讀輸入文字並標記需要保護的區段
2. 辨識上述所有模式的實例，不要把正常的中文語序或作者風格當成 AI 痕跡
3. 重寫每個有問題的部分，優先局部修改
4. 逐項核對修訂後的文字：
   - 大聲朗讀時聽起來自然
   - 自然地改變句子結構
   ...
   - 適當時使用簡單的結構（是/有）
   - 沒有新增事實、經歷、來源、數據或引用
   - 程式碼、URL、數字、名稱、引用與必要揭露保持不變
5. 呈現人性化版本；若使用者沒有要求稽核，避免輸出冗長解說
6. 完成交付後，依「去 AI 味完成後的選擇性詢問」決定是否詢問文字浮水印檢查
...
3. 若發現受保護區段曾被改動，先修正再交付；不要以「看起來更自然」取代內容保真驗證

---

## 品質評分

對改寫後的文字進行 1-10 分評估（總分 50）。這是人性化品質分數，不是浮水印偵測或作者身份證明：
