...
def _find_section_bounds(
    ...
    """Resolve ``--section`` (aliases via ``resolve_section_keys``) or default
    to the related-work section(s), including any ``_2``/``_3`` duplicates."""
    query = section if section else "related"
    matched, available = resolve_section_keys(query, sections)
    return [sections[key] for key in matched], available
...
def analyze(file_path: Path, section: str | None = None) -> list[str]:
    ...
    bounds_list, available = _find_section_bounds(sections, section)
    comment = parser.get_comment_prefix()

    if not bounds_list:
        target = section or "related"
        return [
            f"{comment} ERROR [Severity: Critical] [Priority: P0]: Section not found: {target} "
    ...
    for start, end in bounds_list:
        ...
        for line_no, _raw, text in visible:
            if AUTHOR_ENUM_EN.search(text):
                ...
                if consecutive >= 3:
                    out.extend(
                        [
                            f"{comment} LITERATURE ({doc.lineref(streak_start, line_no - 1)}) "
        ...
        if consecutive >= 3:
            out.extend(
                [
                    f"{comment} LITERATURE ({doc.lineref(streak_start, visible[-1][0])}) "
                    "[Severity: Major] [Priority: P1]: "
                    f"Author/year enumeration detected ({consecutive} consecutive entries)",
                    f"{comment} Suggested: Reorganize the cluster by theme, then compare methods inside the cluster.",
                    f"{comment} Rationale: Listing papers one by one weakens synthesis and hides the research conversation.",
        ...
        if len(fail_ranges) >= 2:
            out.extend(
                [
                    f"{comment} LITERATURE ({doc.lineref(fail_ranges[0][0], fail_ranges[-1][1])}) [Severity: Major] [Priority: P1]: "
            ...
            out.extend(
                [
                    f"{comment} LITERATURE ({doc.lineref(review_start, review_end)}) [Severity: Needs Review] [Priority: P2]: "
        ...
        if tail and not GAP_KEYWORDS_EN.search(tail):
            out.extend(
                [
                    f"{comment} LITERATURE ({doc.lineref(scan_start, end)}) [Severity: Major] [Priority: P1]: "
    ...
    warning_lines = doc.warning_lines(comment)
    if warning_lines:
