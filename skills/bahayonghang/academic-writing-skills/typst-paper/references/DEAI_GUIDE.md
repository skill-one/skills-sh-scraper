...
5. **Defensive speculative explanation** — a result is followed by two or more specific mechanisms without per-mechanism evidence or a discriminating test, then a terminal caveat says the present data do not verify any of them. `may`, `could`, `可能`, or `与……一致` calibrate wording; they do not supply evidence.
...
over-confident wording and undemonstrated applications. Adding `results suggest`,
`may / could`, or `可能/或许` lowers claim strength; it never licenses unsupported
mechanism stacking.
