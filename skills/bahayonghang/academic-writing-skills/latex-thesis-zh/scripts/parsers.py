...
class LatexParser(DocumentParser):
    ...
    SECTION_PATTERNS = {
        ...
        "organization": r"\\(?:chapter|section|subsection){.*(?:组织结构|结构安排|内容安排|技术路线|论文结构)}",
        "summary": r"\\(?:chapter|section|subsection){本章小结}",
...
class TypstParser(DocumentParser):
    ...
    SECTION_PATTERNS = {
        ...
        "organization": r"^={1,3}\s+.*(?:组织结构|结构安排|内容安排|技术路线|论文结构)",
        "summary": r"^={1,3}\s+本章小结",
        "method": r"^=\s+.*(?:方法|原理|设计)",
