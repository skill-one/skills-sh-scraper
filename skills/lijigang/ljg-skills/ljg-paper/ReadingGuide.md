...
成品应让没有读过原论文的人理解作者做了什么、为什么得出这些结论，以及哪些问题仍然没有答案。paper-map 保存来源、证据、取舍理由和检查结果，正文集中解释论文本身。
...
再确认读者的起点：哪些角色、对象和动作必须先分清，哪些背景需要正文补足。理解必要的前提应在第一次用到时交代；讲具体规则之前，先让读者知道它要回答哪个问题、解决什么困难。可以直接解释，也可以借助例子，不强制情境开场、问答体或固定的展开顺序。

保留至少两个能明确指向这篇论文的具体表述，用于核对正文是否仍在解释原论文。它们应包含实际研究对象或贡献，不能只写「模型」「关系」这类通用词。

开头可以直接进入问题或案例。读到前两节时，读者应已知道研究问题与主要贡献，并开始理解作者的思路。
...
检查例子是否让人看见具体条件、动作或关系变化，以及由此产生的后果。把一个抽象术语换成另一个专业名称，仍没有解释发生了什么。例子可以兼顾解释与检验，作用相近时检查是否提供了不同信息。

已经讲清的对象尽量沿用，不为覆盖研究分支频繁切换场景。一个例子说明不了必要问题时，可以更换，但要交代新例子补充什么、与前文如何相连。
...
组件、顺序、状态、反馈、层级或精确映射难以只用文字说明时，可以使用简洁的 ASCII 图或表格。ASCII 图用 Org example 块，宽度不超过 80 显示列。

图后用具体例子解释输入从哪里进入、经过什么、哪个结果因此变化。图表数量按需要决定；重复表示同一关系时优先合并。

## 用自然的中文写清楚

- 写清谁在什么条件下做了什么，产生什么结果。抽象名词有用时再引入，并就地解释。
...
新上下文隔离了写作过程，但不会消除评估者已有的专业知识。评估者先检查理解主线时，是否需要自行补足正文未提供的背景：角色是否分清，术语是否已有必要说明，规则的用途是否交代，结论之间是否省略关键连接。发现缺口时引用原句，写明自己补了什么知识；没有发现时如实记录，不为凑项挑错。
...
这四项分别覆盖表达、内容与理解，不相互抵消。阅读检查验证笔记能否让人理解，不能替代写作者对原文事实的核对。
