...
OIDC token 过期后(HTTP 401)重新执行 `vercel env pull` 即可。切勿提交 `.env.local`。在 GitHub Actions 中使用内置的 `secrets.GITHUB_TOKEN` 即可。
