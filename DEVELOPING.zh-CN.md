...
- 索引只包含内容已落盘的技能:重复技能、上游无快照的技能、抓取失败的技能都不会出现在索引中(记录日志、计入 `Done:` 汇总、下次自动重试)。进程仅在系统性故障(鉴权、排行榜、索引写入)时以非零码退出。
...
`verify.mjs` 是数据集被信任或上传前的门禁:每行可解析、id 唯一、按 installs 降序、字段格式正确、索引行与磁盘目录双向严格对应(每行都有目录、无孤儿目录)、无 `.tmp` 残留。本地快速上手:
...
- **`fetch-skills.yml`**(每日 02:00 UTC + 手动):全量抓取作为每日金丝雀 → 续抓校验(第二次运行新增保存 ≤1%)→ `verify.mjs` → 强制推送每日提交到 [`dist` 分支](README.zh-CN.md#数据在哪里),历史只保留最近 5 个。工作流用长效 `VERCEL_TOKEN` 现场换取新鲜 OIDC token(所需 secrets:`VERCEL_TOKEN`、`VERCEL_ORG_ID`、`VERCEL_PROJECT_ID`——后两项在 `vercel link` 后从 `.vercel/project.json` 复制)。
